...
Aplicación Next.js 16 + Prisma + SQLite para automatizar flujos de marketing de Amarilo: procesamiento de briefs por email (Gmail), generación de Google Sheets, estructura Jira y tráfico de creatividades.

## Stack

- **Framework**: Next.js 16 (App Router)
- **DB**: SQLite vía Prisma 5 (`prisma/schema.prisma`)
- **Auth**: JWT propio con cookies HttpOnly (`src/lib/auth.ts`)
- **UI**: Tailwind CSS v4 + Radix UI + shadcn components
- **Integraciones**: Gmail API (OAuth), Google Sheets/Drive (Service Account), ExcelJS, pdf-parse, node-cron
...
npm run lint         # ESLint
npx prisma db push   # crear/migrar base de datos local
...
1. Copiar variables de entorno:
   ...
   ```
2. Editar `.env.local`:
   - `DATABASE_URL` → `file:./prisma/dev.db` (ya configurado)
...
4. Correr el servidor:
   ...
   npm run dev
   ```
5. Configurar credenciales Google desde la UI en `/dashboard/settings`
...
| `CRON_SECRET` | Token para `/api/cron/*` | cualquier string seguro |

## Despliegue (Railway)

El proyecto usa `Dockerfile` + `railway.toml`. Railway monta un volumen persistente en `/app/prisma/` donde vive `prod.db`.
...
El script `npm start` ejecuta `prisma db push` antes de arrancar Next.js para aplicar migraciones automáticamente en cada deploy.
...
src/
  app/
    api/          # API routes (auth, projects, settings, cron, jira, sheets, traffic)
    dashboard/    # UI protegida
    login/        # Login
  lib/
    auth.ts       # JWT helpers
    prisma.ts     # cliente Prisma singleton
    email/        # Gmail monitor + parser PDF
    sheets/       # Google Sheets client
    brief/        # Parser de briefs PDF
    excel/        # Generación Excel
    jira/         # Estructura Jira
prisma/
  schema.prisma   # modelos: Project, Torre, TrafficEntry, JiraStructure, EmailLog, Settings, User
```

## Notas importantes

- Las credenciales de Gmail y Google Service Account se guardan **en la base de datos** (modelo `Settings`), no como env vars. Se configuran desde la UI.
- El cron de email (`/api/cron/email`) requiere el header `Authorization: Bearer <CRON_SECRET>`.
- En producción, el volumen Railway se monta en `/app/prisma/` — nunca borrar ese volumen o se pierde la BD.
